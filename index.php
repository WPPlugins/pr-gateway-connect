...
/**
 * Plugin Name: PR-Gateway Connect
 * Plugin URI: http://www.pr-gateway.de
 * Description: Publish your articles on Social Networks & +250 press portals
 * Version: 2.3
 * Author: Thomas Kubik
 */
define('PLUGINVERS', '2.3');
...
function PRGConnectMenu() {
    ...
    $pages = array();
    add_menu_page('PR-Gateway Connect', 'Social Connect', 'read', 'prg_connect', null, plugins_url('/images/pr-gateway_logo_menu.png', __FILE__));
    $pages[] = add_submenu_page('prg_connect', 'PR-Gateway Connect', 'Dashboard', 'read', 'prg_connect', 'prg_connect');
    ...
    $pages[] = add_submenu_page('prg_connect', $text['SN'], $text['SN'], 'read', 'prgconfigsocial', 'prgconfigsocial');
    $pages[] = add_submenu_page('prg_connect', $text['SETTINGS'], $text['SETTINGS'], 'read', 'prgpluginconfig', 'prgpluginconfig');

    if (isset($_SESSION['prg_id'])) {
        add_submenu_page('prg_connect', 'Logout', 'Logout', 'read', 'prglogout', 'prglogout');
    }
...
if ($_SERVER['PHP_SELF'] == '/wp-admin/post-new.php' || $_SERVER['PHP_SELF'] == '/wp-admin/post.php') {
    wp_enqueue_script('PRGNewPostJS', plugins_url('/js/prg_connect_NewPost.1_0.js', __FILE__));
...
function prglogout() {
    session_destroy();
    echo '<meta http-equiv="refresh" content="0; URL=?page=prg_connect">';
}
...
function PRGConnect_init() {
    ...
    wp_register_style('PRGStyle', plugins_url('/css/prg_connect.1_0.css', __FILE__));
    wp_register_script('JQuery', plugins_url('/js/jquery.min.1_7.js', __FILE__));
    ...
    wp_register_script('PRGJS', plugins_url('/js/prg_connect.1_0.js', __FILE__));
...
function publish_box() {
    ...
    echo '<div class="misc-pub-section"><span style="padding: 2px 0 1px 20px; background: url(\'' . plugins_url('/images/pr-gateway_logo_menu.png', __FILE__) . '\') no-repeat left center;"><a href="#" class="openPRGCSocialMediaPoster">' . $text['SEND_SN'] . '</a></span></div>';
}
