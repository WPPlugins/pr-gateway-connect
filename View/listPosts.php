    ...
    <div>		
        <div class="pull-right btn-group">
            ...
            <ul class="dropdown-menu" role="menu">
                <li>
                    <a href="?page=prg_connect&filterSet=prg<?php echo!empty($_GET['prgSearch']) ? '&prgSearch=' . $_GET['prgSearch'] : ''; ?>">
                        <input onclick="window.open('?page=prg_connect&filterSet=prg<?php echo!empty($_GET['prgSearch']) ? '&prgSearch=' . $_GET['prgSearch'] : ''; ?>', '_self');" <?php echo ($filters['prg']) ? 'checked="checked"' : '' ?> type="checkbox"> <?php echo $text['TO_PRG_SENT_MSG']; ?>
                    </a>
                </li>
                <li>
                    <a href="?page=prg_connect&filterSet=twitter<?php echo!empty($_GET['prgSearch']) ? '&prgSearch=' . $_GET['prgSearch'] : ''; ?>">
                        <input onclick="window.open('?page=prg_connect&filterSet=twitter<?php echo!empty($_GET['prgSearch']) ? '&prgSearch=' . $_GET['prgSearch'] : ''; ?>', '_self');" <?php echo ($filters['twitter']) ? 'checked="checked"' : '' ?> type="checkbox"> <?php echo $text['TO_TWITTER_SENT_MSG']; ?>
                    </a>
                </li>
                <li>
                    <a href="?page=prg_connect&filterSet=facebook<?php echo!empty($_GET['prgSearch']) ? '&prgSearch=' . $_GET['prgSearch'] : ''; ?>">
                        <input onclick="window.open('?page=prg_connect&filterSet=facebook<?php echo!empty($_GET['prgSearch']) ? '&prgSearch=' . $_GET['prgSearch'] : ''; ?>', '_self');" <?php echo ($filters['facebook']) ? 'checked="checked"' : '' ?> type="checkbox"> <?php echo $text['TO_FACEBOOK_SENT_MSG']; ?>
                    </a>
                </li>
                <li>
                    <a href="?page=prg_connect&filterSet=linkedin<?php echo!empty($_GET['prgSearch']) ? '&prgSearch=' . $_GET['prgSearch'] : ''; ?>">
                        <input onclick="window.open('?page=prg_connect&filterSet=linkedin<?php echo!empty($_GET['prgSearch']) ? '&prgSearch=' . $_GET['prgSearch'] : ''; ?>', '_self');" <?php echo ($filters['linkedin']) ? 'checked="checked"' : '' ?> type="checkbox"> <?php echo $text['TO_LINKEDIN_SENT_MSG']; ?>
                    </a>
                </li>
                <li>
                    <a href="?page=prg_connect&filterSet=tumblr<?php echo!empty($_GET['prgSearch']) ? '&prgSearch=' . $_GET['prgSearch'] : ''; ?>">
                        <input onclick="window.open('?page=prg_connect&filterSet=tumblr<?php echo!empty($_GET['prgSearch']) ? '&prgSearch=' . $_GET['prgSearch'] : ''; ?>', '_self');" <?php echo ($filters['tumblr']) ? 'checked="checked"' : '' ?> type="checkbox"> <?php echo $text['TO_TUMBLR_SENT_MSG']; ?>
                    </a>
                </li>
            </ul>
        </div>
        <button type="button" class="pull-right btn btn-info" id="listPostFBBtn" onclick="window.open('http://developer.pr-gateway.de/wp/feedback.php?lang=<?php echo $userExist->lang; ?>', 'Social Connect Feedback', 'width=650,height=500,scrollbars=yes,toolbar=no,status=no,resizable=no,menubar=no,location=no,directories=no,top=20,left=20');">
                ...
                <tr>
                    ...
                    <td>
                        <a name="preparePost" id="blog-<?php echo $var->ID; ?>" class="preparePost sendBtn">
                            <button id="btnSendToPR" class="btn btn-xs btn-primary" data-color="primary" type="button">
